 ...
 * leecasey
 *
 * This will be the picture for this project.
 */
...
// Reads file and print file name
void writeFile(const Graphics& drawer)
{
...
void loadFile(Graphics& drawer)
{
    ...
    while (inS >> shapesDiff) {
        if (shapesDiff == 'L') {
            Line l;
            inS >> l;
            l.draw(drawer);
        }
        else if (shapesDiff == 'C') {
            Circle c;
            inS >> c;
            c.draw(drawer);
        }
        else if (shapesDiff == 'T') {
            Triangle t;
            inS >> t;
            t.draw(drawer);
        }
        else if (shapesDiff == 'R') {
            Rectangle r;
            inS >> r;
            r.draw(drawer);
        }
        else {
            getline(inS, errorBody);
            drawer.clear();
            cout << "Error in input file: " << shapesDiff << errorBody << endl;
        }
    }
    inS.close();
...
//Returns str with all of its alphabetical characters lowercased
string tolower(string str)
{
